#ifndef RAY_BACKGROUNDLIGHT_H
#define RAY_BACKGROUNDLIGHT_H
...
public: /* Methods: */
    ...
    EmitResult emit () const override {
        ...
        const auto offset = sceneSphere ().center () + sceneSphere ().radius () * (- direction);
        const auto x = discSample.get ().x;
        const auto y = discSample.get ().y;
        const auto vecFromDiscCenter = frame.binormal () * x + frame.tangent () * y;
        const auto position = offset + sceneSphere ().radius () * vecFromDiscCenter;

        const auto emissionPdfW = dirSample.pdfW () * concentricDiscPdfA () * sceneSphere ().invRadiusSqr ();
        const auto directPdfA = dirSample.pdfW ();

        return {intensity (), position, direction,
                direction, emissionPdfW, directPdfA, 1.0};
    }

    RadianceResult radiance (Point, Vector) const override {
        const auto directPdf = uniformSpherePdfW ();
        const auto positionPdf = concentricDiscPdfA () * sceneSphere ().invRadiusSqr ();
