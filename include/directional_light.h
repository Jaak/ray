#ifndef RAY_DIRECTIONAL_LIGHT_H
#define RAY_DIRECTIONAL_LIGHT_H
...
public: /* Methods: */
    ...
    DirectionalLight (const SceneSphere& sceneSphere, Colour intensity, Vector direction)
    ...
    IlluminateResult illuminate (Point pos) const override {
        ...
        return {intensity (), -m_frame.normal (),
                std::numeric_limits<floating>::max (),
                1.0, emissionPdfW, 1.0};
    }
...
private:
    ...
    Point getPoint (floating x, floating y) const {
        const auto offset = sceneSphere ().center () + sceneSphere ().radius () * (- m_frame.normal ());
        const auto vecFromDiscCenter = m_frame.binormal () * x + m_frame.tangent () * y;
