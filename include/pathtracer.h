...
public: /* Types: */
    enum EventType { DIFFUSE, REFLECT, REFRACT, LIGHT };
    ...
    struct Vertex {
        ...
        Vertex(Point pos, Vector normal, Colour col, const Primitive* prim,
               floating pr, EventType event)
            : m_pos{pos}
            , m_normal{normal}
            , m_col{col}
            , m_prim{prim}
        ...
        Vertex(Point pos, Vector normal, Colour col, const Light* light,
               floating pr, EventType event)
            : m_pos{pos}
            , m_normal{normal}
            , m_col{col}
    ...
    struct GeometricFactorCache
        : std::map<std::pair<const Vertex*, const Vertex*>, floating> {
    public: /* Methods: */
...
private: /* Methods: */
    ...
    static inline floating generationPr(const Vertex* from, const Vertex* to) {
        ...
        case LIGHT: {
            ...
            const auto rad =
                from->m_light->radiance(from->m_pos, to->m_pos - from->m_pos);
...
public: /* Methods: */
    ...
    Colour render(Ray ray) { return run(ray); }

    std::unique_ptr<Renderer> clone() const {
        return std::unique_ptr<Renderer>{new Pathtracer(m_scene)};
    }

    Colour operator()(Ray ray) { return run(ray); }
...
private: /* Methods: */
    ...
    void trace(Ray ray, VertexList& vertices) {
        ...
        while (depth < 5) {
            ...
            if (depth > RAY_MAX_REC_DEPTH) {
                if (pr <= rng()) {
                    vertices.emplace_back(point, N2, Colour{0.0, 0.0, 0.0},
                                          prim, 0.0, DIFFUSE);
                    return;
                }
            } else {
            ...
            if (prim->emissive()) {
                vertices.emplace_back(point, N2, objCol / M_PI, prim, pr / M_PI,
                                      DIFFUSE);
                return;
            }
            ...
            case REFRACT: {
                ...
                if (rng() < Re) {
                    ...
                    vertices.emplace_back(point, N2, Re * objCol, prim, pr * Re,
                                          REFLECT);
                    ray = shootRay(point, reflDir);
                    break;
                } else {
                    vertices.emplace_back(point, N2, Tr * objCol, prim, pr * Tr,
                                          REFRACT);
    ...
    VertexList traceLight(floating& lightPA) {
        ...
        vertices.emplace_back(
            emission.position, emission.normal, emission.energy, light,
            clamp(1.0 / (2.0 * M_PI * emission.cosTheta), 0.0, 1.0), LIGHT);
    ...
    Colour radiance(floating eyePA, const VertexList& eyeVertices,
                    floating lightPA, const VertexList& lightVertices) {
        ...
        for (size_t s = 0; s <= NL; ++s) {
            for (size_t t = 1; t <= NE; ++t) {
                ...
                for (size_t i = 0; i < s; ++i)
                    xs.push_back(&lightVertices[i]);
                for (size_t i = 0; i < t; ++i)
                    xs.push_back(&eyeVertices[t - 1 - i]);

                p.resize(s + t + 1, 0.0);
                p[s] = 1.0;
                if (s == 0) {
                    const auto denom =
                        generationPr(xs[1], xs[0]) * gcache(xs[1], xs[0]);
                    p[1] = p[0] * lightPA / denom;
                    if (denom == 0.0)
                        p[1] = 0.0;

                    for (size_t i = s + 1; i < k; ++i) {
                        const auto denom = gcache(xs[i + 1], xs[i]) *
                                           generationPr(xs[i + 1], xs[i]);
                        p[i + 1] = p[i] * gcache(xs[i - 1], xs[i]) *
                                   generationPr(xs[i - 1], xs[i]) / denom;
                        if (denom <= 0.0)
                            p[i + 1] = 0.0;
                    }

                    p[k + 1] = p[k] * gcache(xs[k - 1], xs[k]) *
                               generationPr(xs[k - 1], xs[k]) / eyePA;
                } else {
                    ...
                    p[0] = p[1] * gcache(xs[1], xs[0]) *
                           generationPr(xs[1], xs[0]) / lightPA;
                }
    ...
    // Equation 10.6
    std::vector<Colour> computeAlphaL(floating          lightPA,
                                      const VertexList& lightVertices) const {
        const auto NL = lightVertices.size();
        ...
        for (size_t i = 3; i <= NL; ++i) {
            alpha_L[i] =
                (lightVertices[i - 2].m_col / lightVertices[i - 2].m_pr) *
                alpha_L[i - 1];
        }
        return alpha_L;
    }
    ...
    // Equation 10.7
    std::vector<Colour> computeAlphaE(floating          eyePA,
                                      const VertexList& eyeVertices) const {
        const auto NE = eyeVertices.size();
        ...
        for (size_t i = 2; i <= NE; ++i)
            alpha_E[i] = (eyeVertices[i - 2].m_col / eyeVertices[i - 2].m_pr) *
                         alpha_E[i - 1];
